...
foreach ($BOOKING as $key => $val) {
  $_select = array();
  $_select = array('name','nickname','phone','name_en','zello_id','line_id','username');
  $_where = array();
  $_where['id'] = $val->drivername;
  $res_dv = $this->Main_model->rowdata(TBL_WEB_DRIVER,$_where,$_select);

  $_select = array('topic_th','id', 'province', 'sub', 'main', 'amphur');
  $_where = array();
  $_where['id'] = $val->program;
  $res_ps = $this->Main_model->rowdata(TBL_SHOPPING_PRODUCT,$_where,$_select);
          ...
          <div style="padding: 0px 0px;">
            <table width="100%" class="none-pd">
              ...
              <?php if ($val->transfer_money == 0) {?>
                ...
                $query = $this->db->query('SELECT * FROM pay_history_driver_shopping where order_id = '.$val->id);
