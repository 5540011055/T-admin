...
//echo "</pre>";
$_where = array();
$_where[order_id] = $_GET[order_id];
$this->db->select('price_shopping, price_pay_driver_com, price_income_company_com');
$query_his = $this->db->get_where(TBL_PAY_HIS_DRIVER_SHOPPING,$_where);
$his_pay = $query_his->row();
$num_row_his = $query_his->num_rows();
?>
...
//        $query_planmain = $this->db->get_where(TBL_ORDER_BOOKING_COM,$_where);
        if ($num_row_his > 0) {
          $echo_price = $val->i_price.$unit." = ".number_format($his_pay->price_pay_driver_com,0)." บ.";
          $each_price = $his_pay->price_pay_driver_com;
        }
        else {
          $echo_price = $val->i_price.$unit.' <span class="font-16" style="color:#FF0000;"><i class="fa  fa-circle-o-notch fa-spin 6x" style="color:#FF0000"></i> รอดำเนินการ</span>';
          $each_price = 0;
        }
      }
      else {
        $echo_price = number_format($val->i_price,0)." ".$unit;
        $each_price = $val->i_price;
      }
       $total_price = floatval($total_price) + floatval($each_price);
      ?>
...
//          echo "</pre>";
          ...
          <ons-list-item class="input-items">
            ...
            <label class="center"> 
              <div class="left" style="width: 40%;">
                <span class="font-14"><?=$val->i_buy_amount;?> บ.</span>
              </div>
            </label>
  ...
  <ons-card style="margin-top: 10px; margin-bottom: 10px;">
    ...
    <ons-list-item class="input-items">
      ...
      <div class="center">
        <span class="font-14"><span id="txt_price_total" > <?=$his_pay->price_shopping?> </span>&nbsp; บ.</span>
      </div>
    </ons-list-item>
    ...
    <div align="center">
      ...
      <div class="box-preview-img" id="box_img_profile" style="width: 170px;height: 170px;">
        <img src="../data/fileupload/doc_pay_driver/slip/slip_54.jpg" style="max-width: 100%; height: 170px;display: nones;" id="pv_slip"><br>
